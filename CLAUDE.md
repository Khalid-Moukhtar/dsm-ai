...
15. **NEVER edit a merged or closed PR** — `gh pr edit` on a merged PR is a no-op at best and misleading at worst. Before any `gh pr edit <number>`, verify state: `gh pr view <number> --json state -q .state`. If the output is `MERGED` or `CLOSED`, stop. Do not edit.
16. **Read the Data Map first** — before touching any token-related source code, read `docs/data-maps/THEME_DATA_MAP.md`. The Data Map is the contract.
